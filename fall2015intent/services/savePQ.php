...
//			                       VALUES ('$userID','$projectID','$stageID','$questionID','$queryString','$site','$originalURL','$title','$topResults','$timestamp','$date','$time','$localTimestamp','$localDate','$localTime')";
						...
						$date = $base->getDate();
            $query = "SELECT * FROM queries WHERE userID='$userID' AND stageID='$stageID' AND query='$queryString' AND source='$site' AND `date`='$date'";
            $connection = Connection::getInstance();
			$results = $connection->commit($query);
			...
			if($searchEngine==1)
			{
                if($exists_serp == 0){
                    ...
                    $fileHandle_content = fopen("/www/coagmento.org/htdocs/fall2015intent/webPages/".$filename_content, 'w') or die("file could not be accessed/created");
