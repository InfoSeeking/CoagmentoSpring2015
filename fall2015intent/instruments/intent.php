...
function commitanswer_intention(){
	...
	if(isset($_POST[ "other_text"])){
		$other_text = mysql_escape_string($_POST["other_text"]);
	}


	if(isset($_POST[ "other_reason"])){
		$other_reason = mysql_escape_string($_POST["other_reason"]);
	}
...
function commitanswer_reformulation(){
	...
	$time_start_2 = $_POST['time_start_2'];
	$segmentID1 = $_POST['segmentID1'];
	$segmentID2 = $_POST['segmentID2'];

	$cxn->commit("INSERT INTO video_reformulation_history (userID,projectID,stageID,questionID,segmentID1,segmentID2,time_start_1,time_start_2,`reformulation_reason`,`date`,`time`,`timestamp`

	) VALUES ('$userID','$projectID','$stageID','$questionID','$segmentID1','$segmentID2','$time_start_1','$time_start_2','$reformulation_reason','$date','$time','$timestamp'
...
function commitanswer_save(){
	...
	for($x = 1; $x <= $ninputs; $x+=1){
		...
		$segmentID = $_POST["segmentID_$x"];

		$cxn->commit("INSERT INTO video_save_history (userID,projectID,stageID,questionID,segmentID,time_start,`useful`,`confident`,`date`,`time`,`timestamp`

		) VALUES ('$userID','$projectID','$stageID','$questionID','$segmentID','$time_start','$useful','$confident','$date','$time','$timestamp'
...
function commitanswer_unsave(){
	...
	for($x = 1; $x <= $ninputs; $x+=1){
		...
		$cxn->commit("INSERT INTO video_unsave_history (userID,projectID,stageID,questionID,segmentID,time_start,`unsave_reason`,`date`,`time`,`timestamp`

		) VALUES ('$userID','$projectID','$stageID','$questionID','$segmentID','$time_start','$unsave_reason','$date','$time','$timestamp'
...
function makeTaskData(){
	...
	while($line = mysql_fetch_array($restasks,MYSQL_ASSOC)){
		if(typeMatch($line[$typekey],$questiontype_values['query'])){
			...
			if($prev_query_stamp != ''){
				...
				$question_data[$index]['reformulation']['segmentID1'] = $prev_segmentID;
				$question_data[$index]['reformulation']['segmentID2'] = $new_segmentID;
			...
			$question_data[$index]['intention']['segmentID'] = $line['segmentID'];
			...
			if(!isset($question_data[$index]['save'])){
				...
				$question_data[$index]['save']['segmentID'] = array();
				$question_data[$index]['save']['stamps'] = array();
			}

			array_push($question_data[$index]['save']['stamps'],clean_timestr($line['Elapsed Time']));
			array_push($question_data[$index]['save']['segmentID'],clean_timestr($line['segmentID']));
			...
			if(!isset($question_data[$index]['unsave'])){
				...
				$question_data[$index]['unsave']['segmentID'] = array();
				$question_data[$index]['unsave']['stamps'] = array();
			}

			array_push($question_data[$index]['unsave']['stamps'],clean_timestr($line['Elapsed Time']));
			array_push($question_data[$index]['unsave']['segmentID'],clean_timestr($line['segmentID']));
	...
	$start_string = $question_data[$index-1]['reformulation']['start_stamp'];
	$time_add = 30;

	$start_time = timestrToInt($start_string);
	$start_time += $time_add;

	$question_data[$index-1]['reformulation']['stop_stamp'] = intToPHPTime($start_time);
	...
	for($x=0; $x<=$index;$x+=1){
		...
		if(isset($question_data[$x]['reformulation'])){
			$resresults = $connection->commit("SELECT * FROM video_reformulation_history WHERE userID='$userID' AND projectID='$projectID' AND questionID='$questionID' AND segmentID1='".$question_data[$x]['reformulation']['segmentID1']."'");
			if(mysql_num_rows($resresults) > 0){
...
function getNextTask(){
	...
	foreach($question_data as $index => $data){
		if(isset($data['intention']) && isset($data['intention']['done']) && !$data['intention']['done']){
			...
			$retarr['segmentID'] = $data['intention']['segmentID'];
			return $retarr;
		}
		...
		if(isset($data['reformulation']) && isset($data['reformulation']['done']) && !$data['reformulation']['done']){
			...
			$retarr['segmentID2'] = $data['reformulation']['segmentID2'];
			$retarr['segmentID1'] = $data['reformulation']['segmentID1'];
		...
		if(isset($data['save']) && isset($data['save']['done']) && !$data['save']['done']){
			...
			$retarr['segmentID'] = $data['save']['segmentID'];
			return $retarr;
		}
		...
		if(isset($data['unsave']) && isset($data['unsave']['done']) && !$data['unsave']['done']){
			...
			$retarr['segmentID'] = $data['unsave']['segmentID'];
			return $retarr;
		}
		...
		if(this.name=='other'){
			...
			if(this.checked){
				...
				if(tempret && $(\"input[name=\"+radioname+\"]:checked\").val() == \"No\"){
						textgroup = $(this).attr('pref')+\"_text\";
						tempret = tempret && ($.trim($(\"#\"+textgroup).val()).length > 0);
				}


				var textgroup = $(this).attr('pref')+\"_reason\";
...
function handleCheck(cb){
	...
	if(cb.name =='other'){
		...
		if(cb.checked){
			$(\"#\"+radioname).show();
			var selected = $(\"input[type='radio'][name='\"+pref+\"_radio\"+\"']:checked\");
		...
		var radioname = pref+\"_reasongroup\";
...
if (Util::getInstance()->checkCurrentPage(basename( __FILE__ )))
{
	...
	$cxn = Connection::getInstance();
	$cxn->commit($query);
	$query = "DELETE FROM video_segments WHERE `Event` IS NULL";
	$cxn->commit($query);
	$stageID = $base->getStageID();
		...
		if(file_exists($filedir.$filename)){
			if($taskname=='intention'){
				...
				$render['hiddeninputs']="
					...
					<input type=\"hidden\" name=\"segmentID\" value=\"".clean_timestr($nexttask['segmentID'])."\"/>
				";
				...
				$render['hiddeninputs']="
					<input type=\"hidden\" name=\"time_start_1\" value=\"".clean_timestr($nexttask['start_stamp_prev'])."\"/>
					<input type=\"hidden\" name=\"time_start_2\" value=\"".clean_timestr($nexttask['start_stamp'])."\"/>
					<input type=\"hidden\" name=\"segmentID1\" value=\"".clean_timestr($nexttask['segmentID1'])."\"/>
					<input type=\"hidden\" name=\"segmentID2\" value=\"".clean_timestr($nexttask['segmentID2'])."\"/>
				";
				...
				for($x=1;$x<=count($nexttask['stamps']); $x+=1){
					...
					$render['hiddeninputs'] = $render['hiddeninputs']."<input type=\"hidden\" name=\"segmentID_$x\" value=\"".clean_timestr($nexttask['segmentID'][$x-1])."\"/>";
				}
				$render['javascript'] = "
