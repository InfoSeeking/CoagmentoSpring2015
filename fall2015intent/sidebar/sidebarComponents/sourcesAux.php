...
<?php
        ...
        $query = "SELECT * FROM (SELECT userID,`host`,projectID,MIN(`date`) as `date`,MIN(`time`) as `time`,MIN(`timestamp`) as `timestamp` FROM (SELECT userID,projectID,`date`,`time`,`timestamp`,host,url,status FROM bookmarks UNION ALL SELECT userID,projectID,`date`,`time`,`timestamp`,host,url,status from snippets) a WHERE projectID='$projectID' AND status=1 AND host IS NOT NULL AND url NOT LIKE '%coagmento.org/fall2015intent%' AND url NOT LIKE '%about:blank%' AND url NOT LIKE '%about:home%' AND url NOT LIKE '%about:newtab%' AND url NOT LIKE '%about:addons%' GROUP BY host) d ORDER BY ".$orderBy;
