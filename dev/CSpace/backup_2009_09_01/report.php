...
<?php
	...
	if ($projectID) {
		...
		$title = $line1['title'];
		if ($session) {
			$query = "SELECT * FROM pages WHERE projectID='$projectID' AND date='$session' ORDER BY timestamp";
			...
			$query = "SELECT * FROM pages WHERE projectID='$projectID' ORDER BY timestamp";
		...
		if ($session) {
			$query = "SELECT * FROM pages WHERE date='$session' ORDER BY timestamp";
