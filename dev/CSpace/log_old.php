...
?>
		...
		<table class="table" border=0>
			...
			<select name="objects">
		      ...
		      <option value="pages">Documents</option>
	...
	switch ($objects) {
		case 'pages':
			if ($projectID) {
				...
				$title = $line1['title'];
				if ($session) {
					$query = "SELECT * FROM pages WHERE userID='$userID' AND projectID='$projectID' AND date='$session' ORDER BY timestamp";
					...
					$query = "SELECT * FROM pages WHERE userID='$userID' AND projectID='$projectID' ORDER BY timestamp";
				...
				if ($session) {
					$query = "SELECT * FROM pages WHERE userID='$userID' AND date='$session' ORDER BY timestamp";
				...
				if(isset($_GET[$pageID])) {
					if (isset($_GET['targetProjectID'])) {
						...
						if ($targetProjectID>0) {
							$query1 = "UPDATE pages SET projectID='$targetProjectID' WHERE userID='$userID' AND pageID='$pageID'";
							$results1 = mysql_query($query1) or die(" ". mysql_error());
						}
						else
			...
			while ($line = mysql_fetch_array($results, MYSQL_ASSOC)) {	
				...
				if(isset($_GET[$queryID])) {
					if (isset($_GET['targetProjectID'])) {
						$targetProjectID = $_GET['targetProjectID'];
						if ($targetProjectID>0) {
							$query1 = "UPDATE queries SET projectID='$targetProjectID' WHERE userID='$userID' AND queryID='$queryID'";
							$results1 = mysql_query($query1) or die(" ". mysql_error());
						}
						else
							echo "<font color=red>Please make a valid selection.</font>\n";
			...
			while ($line = mysql_fetch_array($results, MYSQL_ASSOC)) {	
				...
				if(isset($_GET[$noteID])) {
					if (isset($_GET['targetProjectID'])) {
						$targetProjectID = $_GET['targetProjectID'];
						if ($targetProjectID>0) {
							$query1 = "UPDATE annotations SET projectID='$targetProjectID' WHERE userID='$userID' AND noteID='$noteID'";
							$results1 = mysql_query($query1) or die(" ". mysql_error());
						}
						else
							echo "<font color=red>Please make a valid selection.</font>\n";
