					...
					if($hasThumb == NULL || $hasThumb == -1 ) {
						...
						while($line = mysql_fetch_array($pageResult)) {
							...
							$thumb = $line['fileName'];
						  	...
						  	if ($value == $id) {  		
					    		echo "<div class='wrapper'><a class='thumbnail_small2' href=".$row['url']." target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a></div>";	
							}
						}
					}
				}
				...
				if($hasThumb == NULL || $hasThumb == -1 ) {
					...
					while($line = mysql_fetch_array($pageResult)) {
						...
						$thumb = $line['fileName'];
					  	...
					  	if ($value == $id) {  		
				    		echo "<div class='summary cf'><div class='wrapper'><a class='thumbnail_small2' href=".$row['url']." target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a></div>";	
						}
					}
				}
			}
		...
		if($hasThumb == NULL || $hasThumb == -1 ) {
			...
			while($line = mysql_fetch_array($pageFirstResult)) {
				...
				$thumb = $line['fileName'];
			  	...
			  	if ($value == $x) {  		
		    		echo "<div class='summary cf'><h4>Comparing Document:</h4><div class='wrapper'><a class='thumbnail_small2' href=".$row['url']." target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a></div></div>";	
				}
			}
		}
	}
			...
			if($hasThumb == NULL || $hasThumb == -1 ) {
				...
				while($line = mysql_fetch_array($pageResult)) {
					...
					$thumb = $line['fileName'];
				  	...
				  	if ($value == $id) {  		
			    		echo "<div class='wrapper'><a class='thumbnail_small2' href=".$row['url']." target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a>";	
					}
				}
			}
		}
