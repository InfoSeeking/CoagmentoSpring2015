			...
			if($hasThumb == NULL || $hasThumb == -1 ) {
				...
				while($line = mysql_fetch_array($pageFirstResult)) {
					...
					$thumb = $line['fileName'];
				  	...
				  	if ($value == $x) {  		
			    		echo "<div class='summary cf'><h4>Comparing Document:</h4><div class='wrapper'><span class='remove' onclick='removeID(".$value.")' href='javascript:void(0);'><img id='remove-icon' src='remove_icon2.png'></span><a class='thumbnail_small2' href=".$row['url']." target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a></div></div>";	
					}
				}
			}
		}
				...
				if($hasThumb == NULL || $hasThumb == -1 ) {
					...
					while($line = mysql_fetch_array($pageResult)) {
						...
						$thumb = $line['fileName'];
					  	...
					  	if ($value == $checked) {
					  		echo "<div class='wrapper'><span class='remove' onclick='removeID(".$value.")' href='javascript:void(0);'><img id='remove-icon' src='remove_icon2.png'></span>";
						    echo "<a class='thumbnail_small2' href='".$row['url']."' target='new'><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></a></div>";	
						}
					}	
				}
			}
			...
			if($hasThumb == NULL || $hasThumb == -1 ) {
				...
				while($line = mysql_fetch_array($pageResult)) {
					...
					$thumb = $line['fileName'];
				  	...
				  	if ($value == $checked) {
					    echo "<tr><th><div class='wrapper'><span class='remove' onclick='removeID(".$value.")' href='javascript:void(0);'><img id='remove-icon' src='remove_icon2.png'></span><img width='100px' height='100px' src='http://coagmento.org/CSpace/thumbnails/small/".$thumb."' /></div></th>";	
					    echo "<th><a href=".$row['url']." target='new'>".$row['title']."</a></th></tr>";
					}
				}	
			}
